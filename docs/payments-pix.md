...
- Webhook Mercado Pago (`POST /api/payments/pix/webhooks/mercado-pago`) com HMAC **oficial manual** (docs Without SDKs, `data.id` lowercase) como fonte de verdade + SDK `mercadopago-sdk` só diagnóstico; `GET /v1/orders/{id}`; simulação → `SimulatorEvent` 200
